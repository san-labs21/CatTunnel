...
# ==== Persiapkan Direktori Xray (di /usr/local/etc/)
...
# ==== Install SSL Certificate (simpan di /usr/local/etc/xray/)
echo -e "${CREDITS}"
...
# ==== Download Konfigurasi (simpan di /usr/local/etc/xray/)
echo -e "${CREDITS}"
echo -e "Mengunduh konfigurasi Xray dan Nginx..."
wget -q "${GITHUB}tools/config.json" -O /usr/local/etc/xray/config.json
wget -q "${GITHUB}tools/xray.conf" -O /etc/nginx/conf.d/xray.conf

# ==== Update konfigurasi Nginx untuk path certificate yang baru
sed -i 's|/etc/xray/xray.crt|/usr/local/etc/xray/xray.crt|g' /etc/nginx/conf.d/xray.conf
sed -i 's|/etc/xray/xray.key|/usr/local/etc/xray/xray.key|g' /etc/nginx/conf.d/xray.conf

# ==== Update Systemd Service untuk path config yang baru
if [ -f "/etc/systemd/system/xray.service" ]; then
  sed -i 's|/etc/xray/config.json|/usr/local/etc/xray/config.json|g' /etc/systemd/system/xray.service
  # Tambahkan parameter performa
  sed -i '/\[Service\]/a LimitNOFILE=1000000\nLimitNPROC=10000' /etc/systemd/system/xray.service
  systemctl daemon-reload
fi

# ==== Set permissions untuk direktori baru
...
if systemctl is-active --quiet xray; then
  ...
  # Cek apakah Xray bisa membaca config
  if /usr/local/bin/xray test -config /usr/local/etc/xray/config.json; then
    echo -e "Konfigurasi Xray: ${green}Valid${NC}"
  else
    echo -e "Konfigurasi Xray: ${red}Tidak Valid${NC}"
    exit 1
  fi
else
...
echo -e "Xray config: /usr/local/etc/xray/config.json"
echo -e "SSL certificates: /usr/local/etc/xray/xray.{crt,key}"
